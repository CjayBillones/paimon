  ...
  async execute(message, args) {
    try {
      ...
      if(args.length == 0) {
        ...
        } else { message.channel.send("🔴 Failed to link UID. Please contact paimon devs.") }
      }
    } 
    catch {}
  }
...
async function linkUID(message, genshinUID) {
  ...
  if(userExists) {
    message.channel.send(`UID: ${genshinUID} is already linked to another user.`);
    return status
  }
  ...
  embed.addFields(
    {
      ...
      value: "Make sure that you placed the right UID. This can be seen at the bottom right corner ingame."
    }
  )
